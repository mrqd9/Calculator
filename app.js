/* Adding Calculator Pro | Copyright (C) 2026 mrqd9 | GNU GPL v3 or later */
...
const InputController = {
  config: {
    operators: ['+', '−', '×', '÷'],
    ghostChars: [' ', '+', '−', '×', '÷', '='],
    regex: { isPercent: /^[\+\−]?\d+(\.\d+)?%$/, splitSegments: /[\+\−\×\÷%]/, cleanNum: /[, ]/g }
  },
  Cursor: {
    ensureFocus() {
    ...
    setCaret(offset) {
      ...
      const range = document.createRange();
      range.setStart(textNode, offset); range.setEnd(textNode, offset);
      const sel = window.getSelection();
      sel.removeAllRanges(); sel.addRange(range);
      this.renderVisual();
    },
    renderVisual() {
      ...
      if (rects.length > 0) rect = rects[rects.length - 1];
        ...
        range.insertNode(span); rect = span.getBoundingClientRect(); span.remove();
      }
    ...
    enforceConstraints() {
      ...
      let match;
      const resRegex = /=\s*[−-]?\s*[\d.,]+/g;
      while ((match = resRegex.exec(text)) !== null) {
        if (newOffset > match.index && newOffset < match.index + match[0].length) newOffset = match.index + match[0].length;
      }
    ...
    tokenize(rawFull) {
      ...
      for (let part of parts) {
        ...
        if (["+", "−", "×", "÷"].includes(restored)) {
          if (curNum !== null) { initTokens.push(curNum); curNum = null; }
          initTokens.push(restored);
        } else if (restored === "%") {
          if (curNum !== null) {
            if (typeof curNum === 'object' && curNum.isPercent) { curNum.text += "%"; curNum.count++; }
            else curNum = { text: curNum + "%", value: 0, isPercent: true, rawNum: parseFloat(curNum), count: 1 };
          }
        } else {
  ...
  Insert: {
    handle(char, type) {
      ...
      if (type === 'op' && char === '%' && offset > 0) {
        ...
        if (match) { text = text.slice(0, offset - match[0].length) + text.slice(offset); offset -= match[0].length; }
      }
      ...
      if (type === 'op' && offset < text.length) {
        ...
        if (match && !((match[1] === '−' || match[1] === '-') && (char !== '−' && char !== '-'))) {
          text = text.slice(0, offset) + text.slice(offset + match[0].length);
        }
      }

      if (type === 'op') {
        const match = text.slice(0, offset).match(/([\+\−\×\÷])\s*([\+\−\×\÷])?\s*$/);
        if (match) {
          if (match[2] || !((char === '−' || char === '-') && match[1] !== '−' && match[1] !== '-')) {
            text = text.slice(0, offset - match[0].length) + text.slice(offset); offset -= match[0].length;
          }
        }
      }
    ...
    validate(fullText, char, type, offset) {
      ...
      if (/=\s*[−-]?\s*[\d.,]+$/.test(textBefore)) {
        if (/^\s*[−-]?\s*[\d.,]+$/.test(textBefore.split("=").pop()) && (type === 'num' || type === 'dot')) return false;
      }
      ...
      if (type === 'op') {
        if (char === '%' && (!/[\d.]/.test(prevChar) || prevChar === '%' || nextChar === '%')) return false;
      }
      if (type === 'dot') {
        const lastSeg = textBefore.split(InputController.config.regex.splitSegments).pop();
        if (lastSeg.includes('.')) return false;
        if (lastSeg.trim() === "") { this.handle("0", "num"); this.handle(".", "dot"); return false; }
      }
      return true;
    },
    applyPercent() {
      ...
      if (InputController.config.regex.isPercent.test(raw)) {
        const gSum = Utils.getGrandSum();
        if (gSum !== 0) DOM.liveInput.innerText += Utils.formatIN(Utils.toBillingString(gSum));
      }
    ...
    backspace() {
      ...
      const sel = window.getSelection();
      let offset = sel.rangeCount > 0 ? sel.getRangeAt(0).startOffset : text.length;
      if (offset === 0) return false;

      const atomicMatch = text.substring(0, offset).match(/=\s*[−-]?\s*[\d.,]+$/);
      if (atomicMatch) {
        if (/^\s*[−-]?\s*[\d.,]+$/.test(atomicMatch[0].split("=").pop())) {
          const newText = text.slice(0, offset - atomicMatch[0].length) + text.slice(offset);
          DOM.liveInput.innerText = newText;
          InputController.Format.process(newText, offset - atomicMatch[0].length);
          return true;
        }
      }
  ...
  Format: {
    process(text, cursor) {
      let clean = text.replace(InputController.config.regex.cleanNum, "");
      clean = this.updateChains(clean);
      const formatted = this.formatString(clean);
      const meaningfulIndex = text.substring(0, cursor).replace(InputController.config.regex.cleanNum, "").length;

      DOM.liveInput.innerText = formatted;
      let newOffset = 0, charCount = 0;
      if (meaningfulIndex > 0) {
        for (let i = 0; i < formatted.length; i++) {
          if (!", ".includes(formatted[i])) charCount++;
    ...
    updateChains(raw) {
      ...
      let newStr = segs[0], cumul = segs[0];
      for (let i = 1; i < segs.length; i++) {
        let res = evaluate(cumul);
        if (res === "Error") return raw;
        let resStr = Utils.toBillingString(res).replace(/\.00$/, "");
        newStr += "=" + resStr;
        let cur = segs[i], match = cur.match(/^\s*[−-]?\s*[\d.]+/);
        if (match) { newStr += cur.substring(match[0].length); cumul = resStr + cur.substring(match[0].length); }
        else { newStr += cur; cumul = resStr + cur; }
      }
...
function calculatePercentageValue(t, i, tokens) {
  let val = t.rawNum;
  for (let k = 0; k < t.count; k++) val /= 100;
  ...
  if (i > 0) {
    ...
    if ((prev === "+" || prev === "−" || prev === "-") && i > 1) {
      ...
      if (typeof prevPrev === 'string' && !["+", "−", "-", "×", "*", "÷", "/"].includes(prevPrev)) {
         let sub = tokens.slice(0, i - 1);
         let run = evaluate(sub);
         if (run !== "Error") calc = Utils.cleanNum(run * (t.rawNum / 100));
      }
    }
  }
  return calc;
...
function evaluate(tokens = STATE.tokens) {
  ...
  let exp = tmp.map(t => typeof t === "object" ? t.value : t).join(" ")
    .replace(/×/g, "*").replace(/÷/g, "/").replace(/−/g, "-");
  try {
    let res = new Function("return " + exp)();
    return (!isFinite(res) || isNaN(res)) ? "Error" : Utils.cleanNum(res);
  } catch { return "Error"; }
}
...
function tap(fn) {
  let res = fn();
  if (res !== false) Utils.vibrate(30);
  if (STATE.currentPressedBtn) {
    const b = STATE.currentPressedBtn; b.classList.add("pressed");
    setTimeout(() => b.classList.remove("pressed"), 40); STATE.currentPressedBtn = null;
  }
}
...
window.copyToClipboard = () => {
  ...
  let text = "SUMMARY\n";
  rows.forEach(r => text += `${r.querySelector('.h-exp').innerText.trim()}  ${r.querySelector('.h-res').innerText.trim()}\n`);
...
window.resolveInline = () => {
  ...
  let str = Utils.toBillingString(res).replace(/\.00$/, "");
...
window.enter = () => {
  parseAndRecalculate(false);
  if (!DOM.liveInput.innerText.trim() || !STATE.tokens.length) return false;
  let res = evaluate();
  if (res === "Error") return false;
  ...
  if (exp.includes("=")) {
     const lastEq = exp.lastIndexOf("=");
     if (Math.abs(parseFloat(exp.substring(lastEq + 1).replace(/,/g, "")) - res) < 1e-7) exp = exp.substring(0, lastEq).trim();
  }
  exp = exp.replace(/[\s\+\−\×\÷\.\-]+$/, "");
  if (!exp) return false;

  let resTxt = Utils.formatIN(Utils.toBillingString(res));
  if (resTxt.length > 16) resTxt = Number(res).toExponential(8);
  
  row.innerHTML = `<span class="h-exp">${exp} =</span><span class="h-res ${res < 0 ? 'negative' : ''}">${resTxt}</span><div class="swipe-arrow"></div>`;
...
function recalculateGrandTotal() {
  ...
  let txt = Utils.formatIN(Utils.toBillingString(sum));
...
function clearAll() {
  if (STATE.tokens.length === 0 && DOM.history.innerHTML.trim() === "") return false;
  Utils.vibrate(85);
  if (DOM.history.innerHTML.trim()) {
    let arch = safeLoad("calc_archive", []);
    let data = Array.from(document.querySelectorAll(".h-row")).map(r => ({ exp: r.querySelector(".h-exp").innerText, res: r.querySelector(".h-res").innerText, val: Number(r.dataset.value) }));
    let sess = {
      id: STATE.activeSessionId || Date.now(),
      time: new Date().toLocaleString('en-IN', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }),
      data: data, total: DOM.total.innerText, rawTotal: Utils.getGrandSum()
    };
    if (STATE.activeSessionId) arch = arch.filter(i => i.id != STATE.activeSessionId);
    arch.unshift(sess); if(arch.length > 64) arch.pop();
    localStorage.setItem("calc_archive", JSON.stringify(arch));
  }
...
/* --- ARCHIVE & SWIPE --- */
window.showArchive = () => {
  const arch = safeLoad("calc_archive", []);
  ...
  if (arch.length === 0) DOM.archiveList.innerHTML = "<div style='text-align:center; padding:40px; color:#999;'>No history records found</div>";
    ...
    const card = (it, idx) => `
      <div class="archive-item">
        ...
        <div class="archive-data">${it.data.map(r => `<div class="archive-data-row"><span style="color:#888; flex:1;">${r.exp}</span><span class="${r.val < 0 ? 'negative' : ''}" style="font-weight:600;">${r.res}</span></div>`).join("")}</div>
  ...
  DOM.archiveModal.style.display = "block";
  if(!history.state || history.state.modal !== "archive") history.pushState({ modal: "archive" }, "");
  return true;
};

window.restoreSession = (idx) => {
  const s = safeLoad("calc_archive", [])[idx];
  if (!s) return;
  STATE.tokens = []; DOM.liveInput.innerText = ""; DOM.liveTotal.innerText = ""; DOM.history.innerHTML = ""; STATE.activeSessionId = s.id;
  s.data.forEach(d => {
    let r = document.createElement("div"); r.className = "h-row"; r.dataset.value = d.val;
    r.innerHTML = `<span class="h-exp">${d.exp}</span><span class="h-res ${d.val < 0 ? 'negative' : ''}">${d.res}</span><div class="swipe-arrow"></div>`;
...
function enableSwipe(row) {
  ...
  row.addEventListener("touchmove", e => { 
    ...
    let arr = row.querySelector(".swipe-arrow");
  ...
  row.addEventListener("touchend", () => { 
    ...
    if (dx < -th) { row.style.transform = "translateX(-110%)"; Utils.vibrate(30); setTimeout(()=>{ row.remove(); recalculateGrandTotal(); }, 250); }
      ...
      let txt = row.querySelector(".h-exp").innerText.replace(/=\s*$/, "").replace(/,/g, "").trim();
      DOM.liveInput.innerText = txt; InputController.Format.process(txt, txt.length);
...
const CONVERTER = {
  ...
  data: {
    ...
    area: {'m²':1,'ha':10000,'km²':1e6,'sq ft':0.0929,'sq in':0.000645,'ac':4046.86},
    mass: {'kg':1,'g':0.001,'mg':1e-6,'lb':0.453592,'oz':0.0283495},
    volume: {'L':1,'ml':0.001,'m³':1000,'ft³':28.3168,'gal':3.78541,'fl oz':0.02957}
  },
  init() {
    this.pop();
    DOM.convFromUnit.addEventListener("change", () => this.unitChange('from'));
    DOM.convToUnit.addEventListener("change", () => this.unitChange('to'));
  },
  ...
  pop() {
    ...
    this.lastFrom = DOM.convFromUnit.value; this.lastTo = DOM.convToUnit.value;
    this.vis(); this.calc();
  },
  unitChange(side) {
    if (DOM.convFromUnit.value === DOM.convToUnit.value) {
      if (side === 'from') DOM.convToUnit.value = this.lastFrom; else DOM.convFromUnit.value = this.lastTo;
    }
    this.lastFrom = DOM.convFromUnit.value; this.lastTo = DOM.convToUnit.value;
    this.vis(); this.calc();
  },
  vis() {
    document.getElementById("conv-from-name").innerText = this.names[DOM.convFromUnit.value];
    document.getElementById("conv-to-name").innerText = this.names[DOM.convToUnit.value];
  },
  append(c) {
    if (this.cur === "0" && c !== ".") this.cur = c;
    else { if (c === "." && this.cur.includes(".")) return; if (this.cur.length > 10) return; this.cur += c; }
    DOM.convFromVal.innerText = this.cur; this.calc();
  },
  back() {
    this.cur = this.cur.length <= 1 ? "0" : this.cur.slice(0, -1);
    DOM.convFromVal.innerText = this.cur; this.calc();
  },
  calc() {
    const v = parseFloat(this.cur);
    if (isNaN(v)) { DOM.convToVal.innerText = "0"; return; }
    const f = this.data[this.cat][DOM.convFromUnit.value], t = this.data[this.cat][DOM.convToUnit.value];
    let r = (v * f) / t;
    DOM.convToVal.innerText = (Math.abs(r) < 1e-6 && r !== 0 || Math.abs(r) > 1e9) ? r.toExponential(4) : parseFloat(r.toPrecision(8));
  }
};
CONVERTER.init();
...
window.switchUnits = () => {
  const f = DOM.convFromUnit.value, t = DOM.convToUnit.value;
  DOM.convFromUnit.value = t; DOM.convToUnit.value = f;
  CONVERTER.lastFrom = t; CONVERTER.lastTo = f; CONVERTER.vis(); CONVERTER.calc(); return true;
};
...
window.convBack = () => CONVERTER.back();
...
document.querySelectorAll('.btn-key').forEach(btn => {
  ...
  if (btn.classList.contains('cut')) {
    ...
    let cmd = btn.getAttribute('onclick');
    if (cmd) { btn.removeAttribute('onclick'); btn.addEventListener('pointerdown', (e) => { e.preventDefault(); STATE.currentPressedBtn = btn; try { eval(cmd); } catch (err) {} }); }
  }
});
...
window.clearArchive = () => { if(localStorage.getItem("calc_archive") && confirm("Clear entire history archive?")) { localStorage.removeItem("calc_archive"); window.showArchive(); Utils.vibrate(50); } };
window.onpopstate = () => { if (DOM.archiveModal.style.display === "block") DOM.archiveModal.style.display = "none"; if (DOM.converterModal.style.display === "block") DOM.converterModal.style.display = "none"; };
...
(function ThemeSync() {
  ...
  update(m.matches); m.addEventListener('change', e => update(e.matches));
})();
