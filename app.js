...
/* ================= HELPERS ================= */
// Fixed precision issue for very small decimals
function clean(n){
  if (isNaN(n)) return 0;
...
function recalculateGrandTotal(){
  ...
  saveToLocal(); // Auto-save after every change
}
...
function clearAll(){
  ...
  if(confirm("Saara hisab saaf karein?")) {
    tokens = [];
    historyEl.innerHTML = "";
    updateLive();
    recalculateGrandTotal();
...
function cutPressEnd(e){
  ...
  if(!cutLong && back()) if(navigator.vibrate) navigator.vibrate(15);
}
